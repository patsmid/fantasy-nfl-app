...
export async function uploadFantasyProsADP(tipo = 'ppr') {
  ...
  try {
    const adpList = await getFantasyProsADP(tipo);
    ...
    // Índice por normalizedName|position|team
    ...
    for (const p of playersData) {
      ...
      if (!normName || !pos || !team) continue;

      const key = `${normName}|${pos}|${team}`;
      if (!nameIndex.has(key)) nameIndex.set(key, p);
    }
    ...
    for (const src of adpList) {
      ...
      const pos = (src.position || '').toUpperCase();
      const team = normalizeTeam(src.team);
      const adp = Number(src.adp);

      if (!rawName || !pos || !team || isNaN(adp)) {
        notFound.push(`${rawName} (${pos}, ${team})`);
        continue;
      }

      const key = `${normalizeName(rawName)}|${pos}|${team}`;
      ...
      if (!matched && typeof fuzzySearch === 'function') {
        ...
        const [fuzzy] = fuzzySearch(rawName, candidates, { key: p => p.full_name, normalize: normalizeName }) || [];
        if (fuzzy) matched = fuzzy.item || fuzzy;
      }
      ...
      if (insertedIds.has(matched.player_id)) continue;
    ...
    console.log(`📊 Total ADP: ${adpList.length}`);
    ...
    // Borrar previos e insertar nuevos
